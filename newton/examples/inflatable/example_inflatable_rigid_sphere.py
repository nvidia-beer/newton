...
"""
Inflatable Soft Body with Rigid Body Interaction

Demonstrates an inflatable soft body pushing a rigid box upward as it inflates,
...
class Example:
    ...
    def __init__(
        ...
        mesh_min_z = vertices_np[:, 2].min()  # Should be -radius for centered sphere
        mesh_max_z = vertices_np[:, 2].max()  # Should be +radius for centered sphere
        ...
        soft_sphere_top = mesh_max_z + soft_offset  # Top of soft body in world coords
        ...
        # ===== Add soft inflatable sphere =====
        print(f"\n🎈 Adding inflatable soft sphere (BLUE)...", flush=True)
    ...
    def run(self, num_frames: int = 600):
        ...
        for frame in range(num_frames):
            ...
            if frame % 30 == 0:
                ...
                # Compute approximate current radius (max distance from center in XY)
                xy_distances = np.sqrt((particle_positions[:, 0] - sphere_center[0])**2 + 
                                       (particle_positions[:, 1] - sphere_center[1])**2)
                approx_radius = np.max(xy_distances)
...
def main():
    ...
    parser.add_argument('--rigid_mass', type=float, default=0.5,
                        help='Rigid sphere mass (default: 0.5)')
