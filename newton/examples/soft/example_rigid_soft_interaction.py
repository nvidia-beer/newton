...
class RigidSoftInteractionExample:
    ...
    def __init__(
        ...
        # Rigid ball (RED) - falling from above, directly over soft ball
        print(f"\nAdding rigid ball (RED)...")
        builder.add_articulation(key="rigid_ball")
        self.rigid_body_id = builder.add_body(
            xform=wp.transform(wp.vec3(0.0, 0.0, drop_height), wp.quat_identity())
        )
        builder.add_joint_free(self.rigid_body_id)
        ...
        self.model.soft_contact_ke = 5e5   # Increased 10x for stronger pushback
        self.model.soft_contact_kd = 5000.0
        self.model.soft_contact_kf = 5e4
