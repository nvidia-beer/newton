...
"""FEDA double-wishbone rigid car with 4 ANCF FEM tires.

Architecture
------------
  Rigid car    - SolverMuJoCo (Z-up, double_wishbone.xml, 1 MuJoCo world)
  FEM tires x4 - SolverANCFShell (Y-up, n_envs=4, batched)
...
_MJCF_PATH = os.path.join(os.path.dirname(__file__), "assets", "double_wishbone.xml")
...
def _staging_to_xfrc_dw(
    ...
    Forces (f_zu) — ALL three kept: forward traction drives the chassis through
    the DW suspension links; vertical load is two-way; lateral force creates
    understeer/oversteer moments through the geometry.
    Torques — zeroed: spin is kinematically prescribed (no spin DOF on spindle),
    and the other axes have no corresponding rotational DOF on the spindle body.
    """
    env = wp.tid()
    w = staging[env]
...
class Example:
    ...
    def __init__(self, viewer=None, args=None):
        ...
        self._xfrc_stg = wp.zeros(_N_TIRES, dtype=wp.spatial_vector, device=device)
    ...
    def _accumulate_wrenches(self) -> None:
        ...
        self._xfrc_stg.zero_()
        wp.launch(
            _accum_bead_wrench_dw,
            dim=_N_TIRES * self._n_bead,
            inputs=[
                self.ancf_solver.global_f_int,
                self.ancf_solver.node_x,
                self._bead_idx,
                self.solver.xpos,
                self._spindle_mj_arr,
        ...
        wp.launch(
            _staging_to_xfrc_dw,
            dim=_N_TIRES,
            inputs=[self._xfrc_stg, self.solver.xfrc_applied, self._spindle_mj_arr, _COUPLING_ALPHA, self._fz_tare],
            device="cuda:0",
        )
