...
def vertex_triangle_collision_detection_no_triangle_buffers_kernel(
    ...
    """Vertex-triangle collision detection without recording triangle-side buffers (vertex-side only + per-triangle min dist)."""
    v_index = wp.tid()
    v = pos[v_index]
    vertex_buffer_offset = vertex_colliding_triangles_offsets[v_index]
    vertex_buffer_size = vertex_colliding_triangles_offsets[v_index + 1] - vertex_buffer_offset
...
@wp.kernel
def edge_colliding_edges_detection_no_filtering_kernel(
    query_radius: float,
    bvh_id: wp.uint64,
    pos: wp.array(dtype=wp.vec3),
    edge_indices: wp.array(dtype=wp.int32, ndim=2),
    ...
    """Edge-edge collision detection without filtering lists (single query_radius)."""
    e_index = wp.tid()
    e0_v0 = edge_indices[e_index, 2]
    e0_v1 = edge_indices[e_index, 3]
    e0_v0_pos = pos[e0_v0]
