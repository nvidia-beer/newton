...
def compute_element_forces_stiffness(
    ...
    # --- β at element center (xi=eta=zeta=0) for EAS T0 columns ---
    N_c = _shape(0.0, 0.0)
    ...
    # T0 columns: apply β_center to the 5 natural unit-strain basis vectors
    # Mode 0: natural e_d=(1,0,0), e_s=(0,0,0)  → unit E11
    ...
    # F and J0inv at each corner (for ε_zz ANS B-rows)
    N_a = _shape(-1.0, -1.0)
    ...
    # --- ANS shear tying-point F and J0inv (ζ=0) ---
...
# ---------------------------------------------------------------------------
# N-env batched stiffness kernel
# Identical physics to compute_element_forces_stiffness.
# Additional params: n_elems_per_env, n_nodes_per_env.
# tid = env * n_elems_per_env + e_local
# Node state: flat [N*n_nodes]; elem scratch: flat first-dim [N*n_elems].
# Reference / topology arrays (node_x0, elem_nodes, …) are shared.
# ---------------------------------------------------------------------------


@wp.kernel
def compute_element_forces_stiffness_batched(
    ...
    gp_bd: wp.array3d[float],
    gp_bs0: wp.array2d[float],
    gp_b13: wp.array2d[float],
    gp_b23: wp.array2d[float],
    ...
    """dim = N * n_elems_per_env.  Decodes env/e from tid, offsets node reads."""
    e_global = wp.tid()
    env = e_global // n_elems_per_env
    e = e_global % n_elems_per_env
    node_off = env * n_nodes_per_env  # offset into flat node arrays
    ...
    for i in range(24):
        elem_f[e_global, i] = float(0.0)
        for j in range(24):
            elem_K[e_global, i, j] = float(0.0)
                ...
                for node_k in range(4):
                    ...
                    for is_grad_k in range(2):
                        ...
                        for alpha_k in range(3):
                            ...
                            gp_bd[e_global, k, 0] = b_d_mat[0]
                            gp_bd[e_global, k, 1] = b_d_mat[1]
                            gp_bd[e_global, k, 2] = b_d_mat[2]
                            gp_bs0[e_global, k] = b_s_mat[0]
                            gp_b13[e_global, k] = b_s_mat[1]
                            gp_b23[e_global, k] = b_s_mat[2]
                            ...
                            elem_f[e_global, k] = elem_f[e_global, k] + fk

                for k in range(24):
                    bdk0 = gp_bd[e_global, k, 0]
                    bdk1 = gp_bd[e_global, k, 1]
                    bdk2 = gp_bd[e_global, k, 2]
                    bsk0 = gp_bs0[e_global, k]
                    b13k = gp_b13[e_global, k]
                    b23k = gp_b23[e_global, k]
                    for j in range(24):
                        bdj0 = gp_bd[e_global, j, 0]
                        bdj1 = gp_bd[e_global, j, 1]
                        bdj2 = gp_bd[e_global, j, 2]
                        bsj0 = gp_bs0[e_global, j]
                        b13j = gp_b13[e_global, j]
                        b23j = gp_b23[e_global, j]
                        cbj0 = C11 * bdj0 + C12 * bdj1 + C13 * bdj2
                        cbj1 = C12 * bdj0 + C22 * bdj1 + C23 * bdj2
                        cbj2 = C13 * bdj0 + C23 * bdj1 + C33 * bdj2
                        cbj3 = G12 * bsj0
                        ...
                        elem_K[e_global, k, j] = elem_K[e_global, k, j] + kval
