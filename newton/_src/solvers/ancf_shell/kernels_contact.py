...
def apply_ground_contact(
    ...
    slip_scale = wp.tanh(vt_mag / v_reg) / vt_mag
    ...
    # Diagonal stiffness contribution from normal contact (∂fn/∂y = kn)
    wp.atomic_add(K_contact_diag, base + 1, kn)
...
def apply_ground_contact_batched(
    ...
    wp.atomic_add(K_contact_diag, dof_base + 1, kn)
