...
#
# TriMeshCollisionDetector for deformable: surface from get_surface_triangles_and_edges
# (tri mesh or tet boundary). BVH-based vertex–triangle and edge–edge detection.
...
from ...utils.mesh import get_surface_triangles_and_edges
...
class TriMeshCollisionDetector:
    def __init__(
        ...
        # Deformable: derive surface from model (tet boundary or tri mesh) via utils.mesh
        tri_np = model.tri_indices.numpy() if model.tri_indices is not None else None
        tet_np = model.tet_indices.numpy() if model.tet_indices is not None else None
        surface_tris, surface_edges = get_surface_triangles_and_edges(
            tri_np, tet_np, model.tri_count, model.tet_count
        )
        surface_tris = np.asarray(surface_tris, dtype=np.int32)
        surface_edges = np.asarray(surface_edges, dtype=np.int32)
        if surface_tris.ndim == 1:
            surface_tris = surface_tris.reshape(-1, 3)
        if surface_edges.ndim == 1:
            surface_edges = surface_edges.reshape(-1, 2)
        self.tri_count = int(surface_tris.shape[0])
        self.edge_count = int(surface_edges.shape[0])
        self.particle_count = model.particle_count
        self.tri_indices = wp.array(surface_tris, dtype=wp.int32, device=self.device, copy=True)
        edge_4 = np.zeros((self.edge_count, 4), dtype=np.int32)
        edge_4[:, 2:4] = surface_edges
        self.edge_indices = wp.array(edge_4, dtype=wp.int32, device=self.device, copy=True)

        self.lower_bounds_tris = wp.array(shape=(self.tri_count,), dtype=wp.vec3, device=self.device)
        self.upper_bounds_tris = wp.array(shape=(self.tri_count,), dtype=wp.vec3, device=self.device)
        wp.launch(
            kernel=compute_tri_aabbs,
            inputs=[self.vertex_positions, self.tri_indices, self.lower_bounds_tris, self.upper_bounds_tris],
            dim=self.tri_count,
            device=self.device,
        )
        ...
        self.vertex_colliding_triangles = wp.zeros(
            shape=(2 * self.particle_count * self.vertex_collision_buffer_pre_alloc,),
            dtype=wp.int32,
            device=self.device,
        )
        self.vertex_colliding_triangles_count = wp.array(
            shape=(self.particle_count,), dtype=wp.int32, device=self.device
        )
        self.vertex_colliding_triangles_min_dist = wp.array(
            shape=(self.particle_count,), dtype=float, device=self.device
        )
        self.vertex_colliding_triangles_buffer_sizes = wp.full(
            shape=(self.particle_count,),
            value=self.vertex_collision_buffer_pre_alloc,
            dtype=wp.int32,
            device=self.device,
        )
        self.vertex_colliding_triangles_offsets = wp.array(
            shape=(self.particle_count + 1,), dtype=wp.int32, device=self.device
        )
        ...
        if record_triangle_contacting_vertices:
            # Triangle–vertex (inverse) buffers.
            self.triangle_colliding_vertices = wp.zeros(
                shape=(self.tri_count * self.triangle_collision_buffer_pre_alloc,), dtype=wp.int32, device=self.device
            )
            self.triangle_colliding_vertices_count = wp.zeros(
                shape=(self.tri_count,), dtype=wp.int32, device=self.device
            )
            self.triangle_colliding_vertices_buffer_sizes = wp.full(
                shape=(self.tri_count,),
            ...
            self.triangle_colliding_vertices_offsets = wp.array(
                shape=(self.tri_count + 1,), dtype=wp.int32, device=self.device
            )
            self.compute_collision_buffer_offsets(
        ...
        # Required regardless of record_triangle_contacting_vertices.
        self.triangle_colliding_vertices_min_dist = wp.array(
            shape=(self.tri_count,), dtype=float, device=self.device
        )

        # Edge–edge:
        self.edge_colliding_edges = wp.zeros(
            shape=(2 * self.edge_count * self.edge_collision_buffer_pre_alloc,), dtype=wp.int32, device=self.device
        )
        self.edge_colliding_edges_count = wp.zeros(shape=(self.edge_count,), dtype=wp.int32, device=self.device)
        ...
        self.edge_colliding_edges_offsets = wp.array(shape=(self.edge_count + 1,), dtype=wp.int32, device=self.device)
        self.compute_collision_buffer_offsets(self.edge_colliding_edges_buffer_sizes, self.edge_colliding_edges_offsets)
        self.edge_colliding_edges_min_dist = wp.array(shape=(self.edge_count,), dtype=float, device=self.device)

        self.lower_bounds_edges = wp.array(shape=(self.edge_count,), dtype=wp.vec3, device=self.device)
        self.upper_bounds_edges = wp.array(shape=(self.edge_count,), dtype=wp.vec3, device=self.device)
        wp.launch(
            kernel=compute_edge_aabbs,
            inputs=[self.vertex_positions, self.edge_indices, self.lower_bounds_edges, self.upper_bounds_edges],
            dim=self.edge_count,
            device=self.device,
        )
        ...
        # Triangle–triangle intersection (lazy init; not used by deformable solver).
    ...
    def rebuild(self, new_pos=None):
        ...
        wp.launch(
            kernel=compute_edge_aabbs,
            inputs=[self.vertex_positions, self.edge_indices],
    ...
    def triangle_triangle_intersection_detection(self):
        if self.triangle_intersecting_triangles is None:
            self.triangle_intersecting_triangles = wp.zeros(
                shape=(self.tri_count * self.triangle_triangle_collision_buffer_pre_alloc,),
                dtype=wp.int32,
                device=self.device,
            )
        ...
        if self.triangle_intersecting_triangles_offsets is None:
            buffer_sizes = np.full((self.tri_count,), self.triangle_triangle_collision_buffer_pre_alloc)
            offsets = np.zeros((self.tri_count + 1,), dtype=np.int32)
